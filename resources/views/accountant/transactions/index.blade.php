                                    ...
                                    <tbody>
                                        @foreach($transactions as $transaction)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('accountantSalesDetailsIndex', ['id'=>$transaction->sales_id]) }}">{{ $transaction->invoicenumber }}</a></td>
                                                <td>{{ substr($transaction->salesdate, 0, 10) }}</td>
                                                <td>{{ ($transaction->sales_type == 1) ? 'Cash' : 'Credit' }}</td>
                                                <td class="uk-text-primary">{{ number_format($transaction->totalsales, 2) }}</td>
